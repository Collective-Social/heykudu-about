...
export default function PresentationPage() {
  ...
  useEffect(() => {
    ...
    // Fetch standalone presentation HTML to render directly in parent DOM for Chrome native print with 1-to-1 fidelity
    fetch("/wits-strategy-2026-2030.html")
      .then((res) => res.text())
      .then((html) => {
        const parser = new DOMParser();
        const doc = parser.parseFromString(html, "text/html");
        // Extract head styles and links to guarantee 100% identical styling, fonts, and layout
      ...
      <div className="w-screen h-screen bg-gradient-to-br from-[#120826] via-[#630ed4] to-[#0f0919] flex items-center justify-center p-6 text-white font-sans">
        <div className="bg-[#181028]/95 border-2 border-purple-500/40 p-8 rounded-3xl max-w-md w-full shadow-2xl backdrop-blur-xl text-center">
          ...
          <span className="px-3 py-1 rounded-full text-xs font-black bg-purple-500/20 text-purple-200 border border-purple-400/30 uppercase tracking-wider inline-block mb-3">
            Protected Client Briefing
          </span>

          <h1 className="font-display font-black text-2xl text-white mb-2">Wits University Strategy</h1>
          <p className="text-xs text-purple-200 font-bold mb-6">
            Enter your assigned access passcode to view the 2026–2030 Executive Digital Migration Deck.
          </p>
          ...
          <form onSubmit={handleUnlock} className="space-y-4">
            ...
            >
              ...
              <span>{isVerifying ? "Verifying..." : "Unlock Presentation"}</span>
            </button>
          </form>
  ...
  return (
    <>
      {/* Print Overrides CSS for Chrome Native Print */}
      <style jsx global>{`
        @media print {
          @page {
            size: A4 landscape;
      ...
      {/* Screen View Container (Active during normal web navigation) */}
      <div className="screen-view-container w-screen h-screen bg-[#090514] overflow-hidden flex flex-col">
        {/* Top Bar for Executive Navigation & Share Control */}
        <div className="bg-[#630ed4] text-white px-6 py-2.5 flex items-center justify-between z-50 border-b border-white/15 shrink-0 text-xs shadow-md">
          <div className="flex items-center gap-3">
            ...
            <span className="font-extrabold text-white">
              Wits University Top-Down Digital Migration & AI Strategy (2026–2030)
            </span>
          ...
          <div className="flex items-center gap-3">
            ...
            <a
              href="/wits-strategy-2026-2030.html"
              target="_blank"
        ...
        {/* Embedded Fullscreen HTML Presentation */}
        <div className="flex-1 w-full h-full relative">
          <iframe
            src="/wits-strategy-2026-2030.html"
            className="w-full h-full border-0"
            title="Wits University Digital Migration Strategy Presentation"
            allow="fullscreen"
          />
        </div>
      </div>

      {/* Hidden Print Container (Active exclusively during native Chrome/Safari window.print()) */}
      <div
